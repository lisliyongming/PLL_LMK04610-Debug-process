...
PLL2 = 0;
if PLL1 == 1
    disp('In PLL1 Calculation process...');
    freq_in_doubler = 1;
    freq_in = 10e6;
    freq_VCOX = 122.88e6;
    PLL1_RDIV = (1:65536);
    PLL1_NDIV = (1:65536);
    nn = 1;
    for n = 1:length(PLL1_NDIV)
        vco_out_feedback = freq_VCOX / PLL1_NDIV(n);
        for pre = 1:length(PLL1_RDIV)
            clkin_out = freq_in / PLL1_RDIV(pre);
            if (clkin_out == vco_out_feedback)
                result_R(nn) = PLL1_RDIV(pre);
                result_N(nn) = PLL1_NDIV(n);
                nn = nn +1;
            end
        end
    end
    if nn > 1
        result_cal = [result_R;result_N];
        disp('Result(R:N) is below')
        disp(result_cal);
    else
